"""
This script provides the main entry point for evaluating the performance of
...
def main(argv):
    """
    Run an Active Learning performance experiment using Apache Spark.

    The experiment loads and preprocesses the specified dataset, initializes
    the labeled and unlabeled training sets, and iteratively applies the
    selected instance sampling strategy.
    
    At each iteration, the model is retrained using the currently labeled instances
    and evaluated on the independent test set. The resulting performance metrics are
    stored for analysis.
    ...
    argv : list
        ...
        Expected arguments:
            ...
            argv[4] : str, optional
                Output CSV filename. If omitted, ''AL_performance.csv''
                is used.
    ...
    # Parse and validate number of active learning iterations
    try:
        AL_iterations = int(argv[3])
    except ValueError:
        print("Error: Number of Active Learning iterations must be an integer.")
        sys.exit(1)

    # Use the default output filename if none is provided.
    output_filename = argv[4] if len(argv) >= 5 else "AL_performance.csv"
    ...
    print(f"  Number of iterations: {AL_iterations}")
    ...
    # List to store accuracy results in each iteration
    results = []
    ...
    lr_model_fit, accuracy = train_and_evaluate(train_df, test_df, model=lr_model, metric_name="accuracy")

    # Store accuracy for the initial labeled percentage
    labeled_percentage = INITIAL_LABELED_FRACTION * 100
    results.append({
        "method": selection_method,
        "labeled_percentage": labeled_percentage, 
        "accuracy": accuracy,
        "dataset_percentage": percentage_set
    })
    ...
    for i in range(AL_iterations):
        print(f"\n--- Active Learning iteration {i + 1}/{AL_iterations} ---")
        ...
        # Retrain the model using the newly labeled training instances and
        # evaluate its performance on the independent test set.
        lr_model_fit, accuracy = train_and_evaluate(train_df, test_df, model=lr_model, metric_name="accuracy")

        # Store accuracy for the current labeled percentage
        labeled_percentage = labeled_percentage + QUERY_BATCH_FRACTION * 100
        results.append({
            "method": selection_method,
            "labeled_percentage": labeled_percentage, 
            "accuracy": accuracy,
